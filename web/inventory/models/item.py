...
class ItemManager(models.Manager):
    ...
    def with_in_stock_quantities(self, qs=None):
        qs = (qs or self).values('source', 'name', 'raw_items__unit_size').annotate(
            total_pack_quantity=models.Sum('raw_items__raw_incoming_items__in_stock__remaining_pack_quantity')
        ).order_by('source', 'name', 'raw_items__unit_size')
        current_source = None
        current_name = None
        ...
        for item_with_unit in qs:
            if current_source != item_with_unit['source'] or current_name != item_with_unit['name']:
                # complete change of item.
                if current_source:
                    current_quantities.append([current_unit_size, current_quantity])
                    items_with_quantities.append([current_source, current_name, current_quantities])
                current_source = item_with_unit['source']
