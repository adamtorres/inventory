...
class IncomingGroupListForm(forms.Form):
    ...
    total_price = forms.DecimalField(disabled=True)
    total_items = forms.IntegerField(disabled=True)
    total_packs = forms.DecimalField(disabled=True)
    descriptor = forms.CharField(max_length=1024, disabled=True)

    field_order = [
        'selected', 'converted_state', 'source_name', 'action_date', 'total_price', 'total_items', 'total_packs',
        'descriptor']
