...
class SearchCriteriaManager(models.Manager):
    def saved_searches(self, version="0"):
...
class SearchCriteria(sc_models.DatedModel):
    ...
    version = sc_fields.CharField(default="0", null=False)
    ...
    def get_search_queryset(self):
        ...
        include_qs = SourceItem.objects.all()
        if self.version == "0":
            # Version 0 is for the search item page which needs criteria that can be set by the form fields.
            include_qs = self.get_search_queryset_base(self.criteria)
        if self.version == "1":
            # Version 1 is for manually setting the json via the admin page so the search works properly.
            # Will be used for getting the current price for very specific items.
            for include_criteria in self.criteria.get("include") or []:
                include_qs = include_qs & self.get_search_queryset_base(include_criteria)
            for exclude_criteria in self.criteria.get("exclude") or []:
                include_qs = include_qs & self.get_search_queryset_base(exclude_criteria)
        include_qs = include_qs.exclude(models.Q(delivered_quantity__lte=0) | models.Q(extended_cost__lte=0))
        include_qs = include_qs.order_by().order_by('delivered_date', 'source_id', 'order_number', 'line_item_number')
        return include_qs

    def get_search_queryset_base(self, criteria):
        from . import SourceItem
        vts = []
