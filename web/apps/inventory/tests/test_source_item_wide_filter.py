...
class SourceItemWideFilterTestCase(test.TransactionTestCase):
    ...
    def test_example_source(self):
        ...
        self.assertEqual(qs.first().name, "example source")
    ...
    def validate_combined_filter(self, combined_filter, search_terms):
        ...
        self.assertEqual(len(combined_filter.children), len(search_terms))
        self.assertEqual(combined_filter.connector, 'AND')

        name_filter = combined_filter.children[0]
        self.assertIsInstance(name_filter, models.Q)
        self.assertEqual(len(name_filter.children), len(inv_models.SourceItem.wide_filter_fields['name']))
        self.assertEqual(name_filter.connector, 'OR')
        ...
        for name_filter_i, db_field_name in enumerate(inv_models.SourceItem.wide_filter_fields['name']):
            ...
            self.assertEqual(len(name_field_filter.children), len(search_terms[0][1]))  # ('term1', 'term2')
            self.assertIsInstance(name_field_filter, models.Q)
            self.assertEqual(name_field_filter.connector, 'AND')
            for i, name_field_filter_tuple in enumerate(name_field_filter.children):
                with self.subTest(db_field_name=db_field_name, name_field_filter_tuple=name_field_filter_tuple, i=i):
                    ...
                    self.assertEqual(name_field_filter_tuple[0], f"{db_field_name}__icontains")
                    self.assertEqual(name_field_filter_tuple[1], search_terms[0][1][i])
        ...
        self.assertEqual(unit_size_filter[0], f"{search_terms[1][0]}__icontains")
        self.assertEqual(unit_size_filter[1], search_terms[1][1][0])
